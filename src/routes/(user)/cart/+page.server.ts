...
export const actions = {
	...
	ConfirmOrder: async ({ cookies, request }) => {
		...
		return { success: true, lc_orderId: order[0].id, rzp_order_id: rzp_order.id, rzp_order_amount:Math.round(totalAmount)*100 }; // Return a success response with the order ID
	}
};
